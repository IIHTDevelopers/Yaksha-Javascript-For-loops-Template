...
// Function to check if for loop is used correctly
function checkForLoopUsage(ast) {
    let result = 'Pass';
    let feedback = [];
    let forLoopUsed = false;
    ...
    if (!forLoopUsed) {
        result = 'Fail';
        feedback.push("You must use a for loop for iteration.");
    }

    // Detailed logging of the check
    console.log(`\x1b[33mChecking for loop usage\x1b[0m`);
...
// Function to check if the structure of the for loop is correct
function checkForLoopStructure(ast) {
    let result = 'Pass';
    let feedback = [];
    let correctStructureUsed = false;

    ast.body.forEach((node) => {
        if (node.type === 'ForStatement') {
            const { init, test, update } = node;

            // Check if the initialization, condition, and update are present in the for loop
            if (init && test && update) {
                correctStructureUsed = true;
            }
        }
    });

    if (!correctStructureUsed) {
        result = 'Fail';
        feedback.push("You must use the correct for loop structure (initialization; condition; update).");
    }

    // Detailed logging of the check
    console.log(`\x1b[33mChecking for loop structure\x1b[0m`);
...
function gradeAssignment() {
    ...
    testResults.testCaseResults[GUID] = checkForLoopUsage(ast);
    testResults.testCaseResults[GUID + '-for-loop-structure'] = checkForLoopStructure(ast);
